	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const result = await ctx.ui.custom<QuestionnaireResult>((tui, theme, _kb, done) => {
				...
				function saveAnswer(questionId: string, value: string, label: string, wasCustom: boolean, index: number | undefined = undefined) {
		...
		renderCall(args: Record<string, unknown>, theme: Theme) {
			...
			const labels = qs.map((q: Question) => q.label || q.id).join(", ");
		...
		renderResult(result: { content: Array<{ type: string; text?: string }>; details: unknown }, _options: unknown, theme: Theme) {
			...
			const lines = details.answers.map((a: Answer) => {
				if (a.wasCustom) {
